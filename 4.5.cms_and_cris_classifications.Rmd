---
title: "Assessment of CMS and CRIS classification"
...
library(pheatmap);library(wesanderson);library(phytools);library(data.table);library(CMScaller);library(vegan)
```
...
counts <- fread('input_files/All_EPICC_counts.txt')
...
# Concatenate calls
callingRes <- data.frame(Samples=tsam,CMS='',CRIS='')
...
pdf('figures/cms_and_cris_results.pdf')
...
text(x=10,y=42,labels='CMS - All samples',font=2,xpd=T,cex=1.4)
...
mtext(side=2,text='Number of samples',line=2.75,font=2,xpd=T,cex=1.35)
text(x=10,y=42,labels='CRIS - All samples',font=2,xpd=T,cex=1.4)
...
pdf('figures/cmscris_heatmap.pdf')
...
# Plot Figure S1D
pdf('figures/GroupEnrichmentCMSCRIS.pdf')
