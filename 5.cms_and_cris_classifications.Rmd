...
## Figure S4 A-C
```{r plot_cms_and_cris}
# Plot all CMS and CRIS results - Figure S4 A&B
...
for(i in c(1:nrow(patRes))) {
  for(j in c(1:ncol(patRes))) {
    propRes[i,j] <- patRes[i,j]/patRes$NumSam[i]
  }
}

# Plot heatmap of CMS vs CRIS - Figure S4C
pdf('figures/figS4C.cmscris_heatmap.pdf')
...
# Plot Figure S4D
pdf('figures/figS4D.groupenrichmentCMSCRIS.pdf')
...
# Plot all CMS and CRIS results - Figure S4 A&B
pdf('figures/figS4EF.cms_and_cris_confident_results.pdf')
par(mar=c(4.5,4.5,3,1),font=2,font.axis=2,cex.axis=1.35)
cmsplot <- t(patRes[,grep('CMS',colnames(patRes))])
barplot(cmsplot,col=colcms,border=NA,ylim=c(0,40),las=2)
mtext(side=2,text='Number of samples',line=2.75,font=2,xpd=T,cex=1.35)
...
## Figure S4 G&H
```{r plot_cms_and_cris_confident}
# Plot heatmpas of CMS/CRIS centroid distances for each tumour (use C550 as example for Figure S4 G&H)
mybreaks <- seq(0.6,0.8,by=0.005)
pdf('figures/figS4GH.cms_and_cris_distance_heatmaps.pdf',height=17,width=4)
for(pat in patients) {
