...
```{r library}
library(phytools);library(data.table);library(DESeq2);library(org.Hs.eg.db);library(phangorn);library(wesanderson);library(pheatmap);library(STRINGdb);library(metap);library(clusterProfiler);library(stringr)
```
...
for(pat in tinfo$patients) {
  ...
  saveRDS(resdf,file=paste0('results/',pat,'_lambda_data.rds'))
}
```

## Analyse and plot the results of the phylogenetic gene expression analysis (Figure 3C&D and Figure S2)
...
pdf('figures/lambda_analysis_genes.pdf',height=8,width=5)
for(pat in tinfo$patients) {
  resdf <- readRDS(paste0('results/',pat,'_lambda_data.rds'))
...
## Plot recurrent phylogenetic genes (Figure 3E&F)
```{r plot_rec_phylo}
...
pdf('figures/recurrent_genes_lambda.pdf',height=12,width=8.5)
...
## Plot chi-squared test of phylo vs. gene groups (Figure 3G)
```{r plot_chisquared_test}
recsave$Sig <- ifelse(recsave$NumRec>=1,'Sig','NotSig')
pdf('figures/gene_phylo_chiplot.pdf')
...
## Plot the (non-)correlation of sample number x number of sig phylo gene (Figure S3)
```{r plot_correlation}
res <- lm(colSums(recstars!='')~tinfo$numsam);sumres <- summary(res)
pdf('figures/correlation_plot_number_samples_vs_sig_phylo_genes.pdf')
