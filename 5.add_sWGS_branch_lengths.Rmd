...
```{r library}
library(phytools);library(data.table);library(geiger);library(stringr)
tumreg <- c('A','B','C','D')
```

## Load sample data
```{r sample_data_load}
rnasam <- read.table('~/Documents/ThesisOther/ScriptsForThesis/Tables/ListRNAPass.EPICC.txt')[,1]
rnasam <- rnasam[-grep('^C\\d+_E',rnasam)]
rnapats <- unique(gsub('(C\\d+)_\\S+','\\1',rnasam))
dnasam <- readRDS(file='~/Documents/EPICC/Final/0.prepData/dna_sample_data.rds')
```
...
saveRDS(treelist,file='input_files/pgls.trees.rds')
```
...
tinfo <- list(samples=alltreesam,
              ...
              patients=treepats,
              numsam=numpat)
saveRDS(tinfo,file='input_files/pgls.infotrees.rds')
```
