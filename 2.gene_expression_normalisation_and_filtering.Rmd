...
Raw gene counts were merged and filtered for genes on chromosomes 1-22, X, Y and M and
paralogous genes (ending PAR_Y), which were never expressed, were also filtered out

## Filter raw counts for protein coding genes
```{r filter_counts}
raw_counts <- fread('input_files/All_EPICC_counts.allgenes.txt')
geneinfo <- fread("input_files/compiledGeneInfo.txt",header=T)
...
write.table(EPICC_raw,'input_files/All_EPICC_counts.txt',sep='\t',row.names = F,quote = F)
```
...
tpm <- normEPICC
tpm$GeneID <- row.names(tpm);tpm <- tpm[,grep('C\\d+',colnames(tpm))]
write.table(tpm,file='input_files/All_EPICC_tpm.txt')
...
saveRDS(vsd,file='input_files/allgenes.vsd.ensembl.rds')
```

## Filter by expression and correlation with purity
```{r filter_for_expressed_genes}
# Load sample data and gene expression
...
atl1 <- apply(tpm[,tumsam],1,function(x) sum(x>=10))
filens <- tpm$GeneID[which(atl1>=perc5)]
...
saveRDS(filpurens,file='input_files/filteredgenes.5perAL10TPM.pospur.rds')
```
