...
## have permission to access the Hartwig cohort data

```{r library}
library(GenomicRanges);library(data.table);library(dplyr);library(rtracklayer)
library(VariantAnnotation);library(matrixStats);library(pwr);library(wesanderson)
```
...
chain <- import.chain("inputs/hg38Tohg19.over.chain")
hg19ids <- liftOver(idrange, chain)

# Remove loci for which equivalent coordinates in hg19 could not be found
...
saveRDS(lifted,file='results/lifted_ids.rds')
write.table(hg19loci,file='results/hg19loci.txt',row.names=F,col.names=F,sep='\n',quote=F)
```
...
saveRDS(mutmat,file='results/hartwig_qtl_mutmat.rds')
```
...
geneexp <- fread('input/crc_hartwig_allgenes_adjTPM.txt')
...
# Save HW by eQTL data for combining with ATAC-seq data
# Filtering for subclonal Enh mutations
enhmut <- mutidDF[which(mutidDF$Type %in% c('Enh','Both') & mutidDF$ModeladjP<0.01 & mutidDF$MutadjP<0.05 & mutidDF$ClonalityRNA!='clonal'),]
saveRDS(enhmut,'results/enh_subclonal_eqtls.rds')
```
...
## Plot significant HW eQTLs (Figure S7 A-I)
```{r plot_sig_HW_eQTLs}
...
# Plot Figure S7 A-I
pdf('figures/hw_mut_qtls_sigexp.pdf')
...
## Run and plot post-hoc power analysis for HW cohort (Figure S7J)
```{r HW_eqtl_power}
...
# Plot Figure S7J
pdf('figures/HW_eqtl_power_analysis.pdf')
