...
for(pat in patients) {
  ...
  sdmat[[pat]] <- apply(patexp,1,function(x) sd(x))#/(rowMeans(as.matrix(patexp))-min(geneexp))
}
...
# Exploring Group
pdf('figures/fig1BC.exploring_groups.genes.pdf',width=4,height=4)
...
mtext(side=1,at=c(1:4),text=paste0('n=',as.numeric(table(genedf$Group))),line=2)
#boxplot(Mean_Var/Mean_Mean_Exp~Group,data=genedf,col=scales::alpha(ngroupcol,0.5),frame=F,
#        xlab='',ylab='',boxcol=ngroupcol,boxlwd=1.5,medcol=ngroupcol,medlwd=2.5,
#        staplecol=ngroupcol,staplelwd=1.5,whiskcol=ngroupcol,whisklwd=1.5,cex.axis=1.25,
#        outpch=16,outcex=0.5,outcol=scales::alpha(ngroupcol,0.5),las=1)
#mtext(side=1,text='Group',line=3);mtext(side=2,text='Mean SD / Mean Mean Exp',line=3)
#mtext(side=1,at=c(1:4),text=paste0('n=',as.numeric(table(genedf$Group))),line=2)
dev.off()
```
